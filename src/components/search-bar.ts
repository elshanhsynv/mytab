...
const styles = {
  wrapper:
    'group mt-6 flex h-14 w-full max-w-xl items-center gap-3 rounded-2xl border border-violet-400/60 bg-white/10 px-5 shadow-2xl shadow-violet-950/30 backdrop-blur-md transition focus-within:border-violet-300 focus-within:ring-4 focus-within:ring-violet-500/20 sm:mt-8 sm:h-16',
  srOnly: 'sr-only',
  icon: 'flex size-5 shrink-0 text-white/70',
  input:
    'min-w-0 flex-1 bg-transparent text-sm text-white outline-none placeholder:text-white/55 sm:text-base',
  shortcut:
...
export function createSearchBar(onSearch: (query: string) => void): HTMLElement {
  ...
  const emit = debounce((value: unknown) => onSearch(String(value)), APP_CONFIG.SEARCH.DEBOUNCE_MS);
