...
export async function handleContextAction(action: ContextAction, bookmark: BookmarkItem): Promise<void> {
...
export async function hydrateWallpaper(settings: DashboardSettings, legacyWallpaperUrl: string): Promise<void> {
...
function togglePinned(id: string): void {
  ...
  state.set('pinnedIds', pinned.includes(id) ? pinned.filter((pinnedId) => pinnedId !== id) : [...pinned, id]);
}
...
function isContextAction(value: unknown): value is ContextAction {
  return typeof value === 'string' && (CONTEXT_ACTIONS as readonly string[]).includes(value);
}
